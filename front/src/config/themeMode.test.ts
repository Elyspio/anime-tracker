...
describe("readStoredMode", () => {
	it("defaults to dark when nothing is stored", () => {
		expect(readStoredMode(storageWith(null))).toBe("dark");
	});

	it("defaults to dark when storage is unavailable", () => {
		expect(readStoredMode(undefined)).toBe("dark");
	});

	it("defaults to dark when the stored value is corrupt", () => {
		expect(readStoredMode(storageWith("neon"))).toBe("dark");
	});
