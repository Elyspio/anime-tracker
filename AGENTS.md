...
is one click away. Both views open on the **highest-rated** shows first, because a season nobody has
...
- Keycloak nests roles under `realm_access.roles` **and** `resource_access.[client].roles`, and a token
  ...
  flattens both into role claims; the client branch is scoped to `Auth:Audience`, so roles the token
  carries for other clients stay invisible here.
...
deletes the recurring job, so under a rolling update the departing pod would remove the schedule
